...
def linear_predict(data, model):
    ...
    # lin_prediction will be n X n matrix, multiplying data (dxn) with weights (dxn) transpose
    lin_prediction = np.transpose(model['weights']).dot(data)
    # returning the max from each row
    return np.argmax(lin_prediction, axis=0)
...
def perceptron_update(data, model, params, label):
    ...
    # two cases: correct guess --> True, incorrect guess --> False
    # comparison is done with linear-model prediction
    # checking if the label is not equal 
    if(linear_predict(data, model)!=label):
        gold = linear_predict(data, model)
        model['weights'][:,gold][:] = model['weights'][:,gold] - (params['lambda'])*(data)
        model['weights'][:,label][:] = model['weights'][:,label] + params['lambda']*(data)
        return False
...
def log_reg_train(data, labels, model, check_gradient=False):
    ...
    def log_reg_nll(new_weights):
        ...
        nll = 0
        # I will create two functions, one for calculating nll and 
        # second for calculating the gradient, then the main function
        # log_reg_nll will return the both tuples
        
        # TODO fill in your code here to compute the objective value (nll)
        def nll_calculator(dat, n, d):
            num = 0
            for i in range(d):
                num -= np.sum(dat[i, labels==i], 0)
            num = num + np.sum(logsumexp(dat, 0))
            return num
            
        # TODO fill in your code here to compute the gradient
        # compute the gradient
        def gradient_calculator(dat, n, m):
            num2 = np.zeros((n, m))
            temp = np.transpose(np.exp(dat-logsumexp(dat, 0)))
            for j in range(m):
                num2[:, j] = -data.dot(-temp[:, j] + (labels == j))
            return num2
        
        cur_weights = np.transpose(new_weights).dot(data)
        
        # calculating the required negative log likelihood, gradient
        nll = nll_calculator(cur_weights, d, num_classes)
        gradient = gradient_calculator(cur_weights, d, num_classes)
